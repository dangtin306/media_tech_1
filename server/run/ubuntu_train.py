...
def load_ssh_config() -> UbuntuSSHConfig:
    ...
    return UbuntuSSHConfig(
        ...
        remote_main=as_str(
            config.get("ubuntu_qwen_train_main")
            or os.environ.get("UBUNTU_QWEN_TRAIN_MAIN"),
            REMOTE_TRAIN_MAIN,
        ),
        remote_app_dir=as_str(
            config.get("ubuntu_qwen_train_app_dir")
            or os.environ.get("UBUNTU_QWEN_TRAIN_APP_DIR"),
            REMOTE_APP_DIR,
        ),
        remote_log_file=as_str(
            config.get("ubuntu_qwen_train_log_file")
            or os.environ.get("UBUNTU_QWEN_TRAIN_LOG_FILE"),
            REMOTE_LOG_FILE,
        ),
        remote_pid_file=as_str(
            config.get("ubuntu_qwen_train_pid_file")
            or os.environ.get("UBUNTU_QWEN_TRAIN_PID_FILE"),
            REMOTE_PID_FILE,
        ),
    )
...
def build_stop_command(cfg: UbuntuSSHConfig) -> str:
    ...
    target = f"[p]ython {cfg.remote_main}"
    return (
        "bash -lc "
        + shlex.quote(
            f"set +e; "
...
def build_start_command(cfg: UbuntuSSHConfig) -> str:
    ...
    inner = (
        ...
        f": > {remote_log}; "
        f"nohup python {remote_main} --use_4bit --checkpointing >> {remote_log} 2>&1 < /dev/null & "
        f"echo $! > {remote_pid}; "
        f"echo STARTED:$!; "
        f"sleep 1; "
        f"ps -p \"$!\" -o pid=,cmd="
...
def stream_remote_log(cfg: UbuntuSSHConfig, lines: int = 40) -> None:
...
def restart_remote_daemon(
    ...
    try:
        ...
        print(f"[ok] remote train restarted: {cfg.host}:{cfg.port}")
    ...
    if follow_enabled:
        ...
        stream_remote_log(cfg, lines=follow_lines)
...
def status_remote_daemon(cfg: UbuntuSSHConfig, timeout: int = DEFAULT_START_TIMEOUT) -> int:
    ...
    try:
        ...
        if out.strip():
            return 0
        print("[down] remote train is not running")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Restart Qwen LoRA training on Ubuntu via SSH.")
    parser.add_argument("--stop", action="store_true", help="Stop the remote training daemon only.")
    parser.add_argument("--status", action="store_true", help="Check the remote training daemon status.")
